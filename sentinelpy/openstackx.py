...
class OSSession(object):
    ...
    def __init__(self, openstack_ip, username, password):
        ...
        self.identity = self.Identity(self.identity_url, self.username, self.password, domain='default')
        ...
        self.compute = self.Compute(self.compute_url, self.auth_token)
        self.networking = self.Networking(self.networking_url, self.auth_token)
        self.image = self.Image(self.image_url, self.auth_token)
